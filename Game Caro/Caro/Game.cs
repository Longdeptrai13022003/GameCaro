...
namespace VCaro
{
    ...
    class Game
    {
        ...
        public Game()                       //khởi tạo
        {
            ...
            pen = new Pen(Color.Chartreuse);                         //màu để kẻ đường chiến thắng
        ...
        private VMove VlMaxPos(List<VMove> ValueBoard)  //lấy ra nước đi có giá trị lớn nhất
        {
            ...
            ValueBoard[max] = new VMove(rs.x, rs.y, long.MinValue,0);       
            return rs;
        }
        
        
        #region Attack      //các hàm lượng giá điểm tấn công theo các phương
        private long AttackPointHorizontal(int NLine, int NColumn,int player)     
        {
            long Rs = 0;        
            int M = 0;     
            int temp = 0;      
            int E = 0;       
            for (int Count = 1; Count < 7 && NLine + Count < _CaroBoard.LineAmount + 2; Count++)    
            {
                if (_Nodes[NLine + Count, NColumn].NStatus == player)   
                    M++;
                if (_Nodes[NLine + Count, NColumn].NStatus == -player ||  _Nodes[NLine + Count, NColumn].NStatus ==2)   
                {
                    E++;
                    break;
                }
                if (_Nodes[NLine + Count, NColumn].NStatus == 0)    
                {
            ...
            for (int Count = 1; Count < 7 && NLine - Count >= 0; Count++)   
            {
                if (_Nodes[NLine - Count, NColumn].NStatus == player)   
                    M++;
                if (_Nodes[NLine - Count, NColumn].NStatus == -player || _Nodes[NLine - Count, NColumn].NStatus == 2) 
        ...
        #endregion
        #region Defend
        private long DefendPointHorizontal(int NLine, int NColumn, int player)  
        {
            long Rs = 0;
            int M = 0, temp = 0;
            int E = 0;
            for (int Count = 1; Count < 7 && NLine + Count < _CaroBoard.LineAmount; Count++) 
            {
        ...
        {
            if(WinCheck(false)==true)       
            {
                if (_Result == -1) return new VMove(1,depth);       
                else
                    if (_Result == 1) return new VMove(-1,depth);   
                    else return new VMove(0,0);                    
            }
            if (depth == 0) return new VMove(0,0);     
            VMove temp;     
            VMove rs=new VMove();   
            if (player == 1)        
            {
                List<VMove> vb =EValue(1);      

                VMove Bm = new VMove(2, 10);//Gán Bm là giá trị tệ nhất (value = 2,không trùng với value của trường hợp thắng, thua, hòa) và độ sâu sâu nhất để tìm nước đi tệ nhất
                VMove[] hMove = new VMove[hmoveamount];     
                for (int i = 0; i < hmoveamount; i++)  //Trả về hmoveamount giá trị lớn nhất trong list     
                {                                      //vb vì sau mỗi lần lấy giá trị lớn nhất hàm
                    hMove[i] = VlMaxPos(vb);           //VlMaxPos lại gán giá Value = long.Minvalue
                }                                      //cho phần tử vừa được lấy ra
                for (int i = 0; i < hmoveamount; i++) 
                {
                    _Nodes[hMove[i].x, hMove[i].y].NStatus = 1;         
                    _StackMoves.Push(_Nodes[hMove[i].x, hMove[i].y]);       
                    temp = FindCMove(depth - 1, cmoveamount, hmoveamount, -1, alpha, beta); 
                    if (Bm.value > temp.value || (Bm.value == 1 && temp.value == 1 && Bm.depth > temp.depth) || (Bm.value == -1 && temp.value == -1 && Bm.depth < temp.depth))
                    {//Lấy giá trị nhỏ nhất, nếu thắng thì lấy node có độ sâu cao hơn, thua thì lấy độ sâu thấp nhất                           
                    ...
                    if (beta > Bm.value) beta = Bm.value;   
                    if (beta <= alpha) break;
            ...
            }
            {
                ...
                for (int i = 0; i < cmoveamount; i++)
                {
                    ...
                    _StackMoves.Push(_Nodes[cMove[i].x, cMove[i].y]);       
                    temp = FindCMove(depth - 1, cmoveamount, hmoveamount, 1, alpha, beta);   
                    if (Bm.value < temp.value || (Bm.value == 1 && temp.value == 1 && Bm.depth < temp.depth) || (Bm.value == -1 && temp.value == -1 && Bm.depth > temp.depth))
                    { 
                        rs.value = Bm.value = temp.value;
